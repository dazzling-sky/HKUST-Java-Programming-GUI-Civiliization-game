...
public class GameCanvas extends StackPane {
    ...
    public GameCanvas(GameMap gameMap, InfoBar infoBar) {
        ...
        double canvasHeight = gameMap.getHeight() * CELL_WIDTH;
    ...
    private void drawCity(City city, Image flag) {
        ...
        Image cityImage = city.getImage(CELL_WIDTH, CELL_HEIGHT);
        Cell cityLocation = gameMap.getCityLocation(city);
        Cell iconLocation = toCanvasCoordinates(cityLocation);

        canvasLayers[ICON_LAYER].getGraphicsContext2D().drawImage(cityImage, iconLocation.getX(), iconLocation.getY());
        canvasLayers[TEXT_LAYER].getGraphicsContext2D().strokeText(city.getName(), iconLocation.getX(), iconLocation.getY());
        canvasLayers[FLAG_LAYER].getGraphicsContext2D().drawImage(flag, iconLocation.getX(), iconLocation.getY());
    }
    ...
    private void drawLinesBetweenCities(City a, City b) {
        ...
        Player ownerA = gameMap.getCityOwner(a);
        Player ownerB = gameMap.getCityOwner(b);

        Cell cityLocationA = gameMap.getCityLocation(a);
        Cell iconLocationA = toCanvasCoordinates(cityLocationA);

        Cell cityLocationB = gameMap.getCityLocation(b);
        Cell iconLocationB = toCanvasCoordinates(cityLocationB);

        if(ownerA.getName() == ownerB.getName()){
            canvasLayers[LINE_LAYER].getGraphicsContext2D().setLineWidth(2.5);
            canvasLayers[LINE_LAYER].getGraphicsContext2D().setStroke(Color.LIGHTGREEN);
            canvasLayers[LINE_LAYER].getGraphicsContext2D().strokeLine(iconLocationA.getX() + CELL_WIDTH/2, iconLocationA.getY() + CELL_HEIGHT/2,iconLocationB.getX() + CELL_WIDTH/2, iconLocationB.getY() + CELL_HEIGHT/2);
        }
        else{
            canvasLayers[LINE_LAYER].getGraphicsContext2D().setLineWidth(2.5);
            canvasLayers[LINE_LAYER].getGraphicsContext2D().setStroke(Color.RED);
            canvasLayers[LINE_LAYER].getGraphicsContext2D().strokeLine(iconLocationA.getX() + CELL_WIDTH/2, iconLocationA.getY() + CELL_HEIGHT/2,iconLocationB.getX() + CELL_WIDTH/2, iconLocationB.getY() + CELL_HEIGHT/2);
        }
    }
