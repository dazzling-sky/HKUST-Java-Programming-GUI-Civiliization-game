...
public class GameMap {
    ...
    private List<Boolean> isMinisterReadyList;

    public GameMap() {
        players = new ArrayList<>();
        isMinisterReadyList = new ArrayList<>();
        cityLocations = new HashMap<>();
        connectedCities = new HashMap<>();
    }

    public List<Boolean> getIsMinisterReadyList(){
        return isMinisterReadyList;
    }
    public int getWidth() {
        return width;
    }
    ...
    public void loadPlayers(String filename) throws IOException {
        ...
        try (Scanner sc = new Scanner(file)) {
            ...
            if(filename == "save.txt"){
                ...
                for(int i=0; i< players.get(playerTurn).getMinisters().size(); i++){
                    Minister chosenMinister = players.get(playerTurn).getMinisters().get(i);
                    String isMinisterReady = sc.next();

                    boolean isMinisterReadyBoolean = Boolean.valueOf(isMinisterReady);
                    isMinisterReadyList.add(isMinisterReadyBoolean);
                }
